...
all = []
# 世界威士忌79種
for page in range(1, 10):  # 共有9頁
    world_URL = soup.select('li.cat-item-35483995 a')[0]['href'] + 'page/{}'.format(str(page))
    # print(Ireland_URL)
    # IrelandURL = Ireland_URL.format(str(page))
    res = requests.get(url=world_URL, headers=headers)
    res.encoding = "utf-8"
    soup = BeautifulSoup(res.text, 'html.parser')
    ...
    for wh in whisky:#每一款酒的評論連結  主要資訊都在這
        ...
        print(name)
        country = soup.select('a[class="post-lead-category"]')[0].text
        try:
    ...
    a_df.to_csv(r'./whisky/World_whisky.csv', index=False, encoding='utf-8-sig')
    #
