...
for page in range(1,8): #共有7頁
    us_URL = soup.select('li.cat-item-16944253 a')[0]['href'] + 'page/{}'.format(str(page))
    # print(us_URL)
    # IrelandURL = Ireland_URL.format(str(page))
    res = requests.get(url=us_URL, headers=headers)
    res.encoding = "utf-8"
    soup = BeautifulSoup(res.text, 'html.parser')
    whisky = soup.select('div#main-content h2.posttitle')
    img_list = soup.select('div#main-content section.entry')
    ...
    # a_df = df.append(pd.DataFrame(all, columns=['酒名']))

    a_df.to_csv(r'./whisky/US_whisky.csv', index=False, encoding='utf-8-sig')
    #
