...
all = []
#蘇格蘭威士忌1270種
for page in range(65,129): #共有128頁 101~105
    Scotch_URL = soup.select('li.cat-item-1418205 a')[0]['href'] + 'page/{}'.format(str(page))

    # print(Ireland_URL)
    # IrelandURL = Ireland_URL.format(str(page))
    res = requests.get(url=Scotch_URL, headers=headers)
    res.encoding = "utf-8"
    soup = BeautifulSoup(res.text, 'html.parser')
    ...
    for wh in whisky:#每一款酒的評論連結  主要資訊都在這
        ...
        try:
            ...
            try:
                content = soup.select('section[class="entry"]')[0].text.split('What they say')[1].split('What we say')[
                    0].replace("\n", "")
            except IndexError as e:
                try:
                    content = soup.select('section[class="entry"]')[0].text.split('What They say')[1].split('What I say')[
                    0].replace("\n", "")
                except IndexError as e:
                    print(e)
                    content = 'null'
        ...
        try:
            ...
            try:
                comment = soup.select('section[class="entry"]')[0].text.split('What we say')[1].split('Overall')[0].replace(
                "\n", "")
            except IndexError as e:
                print(e)
                comment ='null'
        ...
        try:
            ...
            print(e)
            try:
                info3 = soup.findAll('section.entry h4').text
            except AttributeError as e:
                print(e)
        ...
        if len(info) > 1:
            ...
            try:
                ABV = [m.group().split('ABV')[0]for m in re.finditer(re.compile(r'\d\d\S\sABV'), info2)][0]
                # print(type(ABV))
                # print(ABV)
                Cost = [m.group().split('\n')[0].split('buy')[0].split('for')[0].split('from')[0]for m in re.finditer(re.compile(r'£\d.*\s'), info2)][0]
                # print(Cost)
                Age = 'null'
                factory = 'null'
                Category = 'null'
            ...
            try:
                ...
                # print(type(ABV))
                # print(ABV)
                Cost = [m.group().split('\n')[0].split('buy')[0].split('for')[0].split('from')[0] for m in
                        re.finditer(re.compile(r'£\d.*\s'), info2)][0]
                # print(Cost)
                Age = 'null'
                factory = 'null'
                Category = 'null'
    ...
    #
        ...
        data1.append(Cost)
        all.append(data1)
    # with open(r'./whisky/scotch_log.csv', encoding='utf-8', mode='a')as f:
    #     f.write(all)

    a_df = df.append(pd.DataFrame(all, columns=['酒名', '酒廠', 'url', '年分', '產地', '酒精度', '照片url', '內容', '評論', '價錢']))

    a_df = a_df.drop_duplicates('酒名')
    a_df.to_csv(r'./whisky/Scotch2.csv', index=False, encoding='utf-8-sig')
